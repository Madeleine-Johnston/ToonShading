...
GLuint silShaderProgram;
GLuint decColShaderProgram;
GLuint whiteShaderProgram;
...
glm::vec3 mirror_pos(-5.0f, 1.5f, -5.0f);
...
void init(void) {
	...
	silShaderProgram = rt3d::initShaders("silhouette.vert", "silhouette.frag");
	rt3d::setLight(silShaderProgram, light0);
	rt3d::setMaterial(silShaderProgram, material0);
	uniformIndex = glGetUniformLocation(silShaderProgram, "attConst");
	glUniform1f(uniformIndex, attConstant);
	uniformIndex = glGetUniformLocation(silShaderProgram, "attLinear");
	glUniform1f(uniformIndex, attLinear);
	uniformIndex = glGetUniformLocation(silShaderProgram, "attQuadratic");
	glUniform1f(uniformIndex, attQuadratic);
...
void draw(SDL_Window * window) {
	...
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-10.0f, 0.1f, -4.0f));
	...
	rt3d::setUniformMatrix4fv(toonShaderProgram, "projection", glm::value_ptr(projection));
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(-5.0f, 0.1f, -4.0f));
	mvStack.top() = glm::scale(mvStack.top(), glm::vec3(20.0, 20.0, 20.0));
	rt3d::setUniformMatrix4fv(toonShaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	rt3d::setMaterial(toonShaderProgram, material0);
	rt3d::drawIndexedMesh(meshObjects[2], toonIndexCount, GL_TRIANGLES);
	...
	rt3d::setUniformMatrix4fv(toonShaderProgram, "projection", glm::value_ptr(projection));
	mvStack.push(mvStack.top());
	mvStack.top() = glm::translate(mvStack.top(), glm::vec3(0.0f, 0.1f, -4.0f));
	mvStack.top() = glm::scale(mvStack.top(), glm::vec3(20.0*1.1, 20.0*1.1, 20.0*1.1));
	rt3d::setUniformMatrix4fv(toonShaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	rt3d::setMaterial(toonShaderProgram, material0);
	rt3d::drawIndexedMesh(meshObjects[2], toonIndexCount, GL_TRIANGLES);
	mvStack.pop();
	...
	mvStack.top() = glm::scale(mvStack.top(), glm::vec3(20.0, 20.0, 20.0));
	rt3d::setUniformMatrix4fv(toonShaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	rt3d::setMaterial(toonShaderProgram, material2);
	rt3d::drawIndexedMesh(meshObjects[2], toonIndexCount, GL_TRIANGLES);
	mvStack.pop();

	glDisable(GL_STENCIL_TEST);

////Procedural Geometry - code compiles but crashes during runtime
	...
	//glUniform1f(u_color1, float(0)); // black colour
	//glUniform1f(u_offset1, 0.65f); // offset along normal
	//glDrawElements(GL_TRIANGLES, 1, GL_UNSIGNED_INT, NULL); // draw mesh

	//glCullFace(GL_CW); // enable culling of back faces
	//glDepthMask(GL_FALSE); // disable writes to Z-buffer
	//glUniform1f(u_color1, float (1)); // white color
	...
	//glDepthMask(GL_TRUE); // enable writes to Z-buffer
	//mvStack.push(mvStack.top());
	//mvStack.top() = glm::translate(mvStack.top(), glm::vec3(2.0f, 0.1f, -4.0f));
	//mvStack.top() = glm::scale(mvStack.top(), glm::vec3(15.0, 15.0, 15.0));
	//rt3d::setUniformMatrix4fv(silShaderProgram, "modelview", glm::value_ptr(mvStack.top()));
	//rt3d::setMaterial(silShaderProgram, material0);
	//rt3d::drawIndexedMesh(meshObjects[2], toonIndexCount, GL_TRIANGLES);
	//mvStack.pop();
